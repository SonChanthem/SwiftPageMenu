...
final public class TabMenuView: UIView {
    ...
    init(options: PageMenuOptions) {
        ...
        case let .underline(barColor, _, radius):
            ...
            underlineView.layer.cornerRadius = radius
...
extension TabMenuView {
    ...
    func scrollCurrentBarView(_ index: Int, contentOffsetX: CGFloat, progress: CGFloat) {
        ...
        case let .roundRect(rectColor, _, _, _, _):
            self.cursorView.color = rectColor
        case let .underline(barColor, _, _):
            self.cursorView.color = barColor
        }
        ...
        if let currentCell = collectionView.cellForItem(at: currentIndexPath) as? TabMenuItemCell, let nextCell = collectionView.cellForItem(at: nextIndexPath) as? TabMenuItemCell {
            // assing new option
            currentCell.options = self.options
            nextCell.options = self.options
